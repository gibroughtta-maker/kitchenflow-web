...
// --- Cravings (优先直连 Supabase) ---

export async function getCravings(): Promise<import('../types').Craving[]> {
  // 1. 优先直连 Supabase
  if (isSupabaseConfigured() && supabase) {
    try {
      const deviceId = getDeviceId();
      ...
      if (!error && data) {
        // [Sync Fix] Migration for Cravings
        if (data.length === 0) {
          const localData = storage.getCravings();
          if (localData.length > 0) {
            console.log('🚀 Migrating local CRAVINGS to Supabase...', localData);
            await setCravings(localData);
            return localData;
          }
        }
  ...
  // 2. 降级
  return Promise.resolve(storage.getCravings());
}

export async function setCravings(items: import('../types').Craving[]): Promise<void> {
  // [Fix] Ensure all items have valid UUIDs
  let hasChanges = false;
  const sanitizedItems = items.map(item => {
    if (!item.id || !isUUID(item.id)) {
  ...
  if (hasChanges) {
    console.log('Sanitized CRAVING items with new UUIDs:', sanitizedItems);
  }
  ...
  if (isSupabaseConfigured() && supabase) {
    try {
      ...
      if (currentIds.length > 0) {
        ...
        await supabase.from('cravings').delete().eq('device_id', deviceId);
      }

      // Success - no need to fall back
      storage.setCravings(sanitizedItems);
      return;
    } catch (e) {
      console.warn('Supabase direct write failed (Cravings), falling back:', e);
    }
  }
